...
from utilities.utilities import BETS, DARTS_COMB
...
async def choose_darts(callback: CallbackQuery, balance: list[int]):
    ...
    balance[0] -= bet
    balance[0] += round(bet * DARTS_COMB[hid_num])
    rep_text = f"{L_RU_cas[f'darts_{hid_num}']}\n{L_RU_cas['balance'].format(balance[0])}"
...
async def callback_play_again_basket(callback: CallbackQuery, balance: list[int]):
    ...
    if BETS[callback.from_user.id] > balance[0]:
        ...
        await choose_darts(callback, balance)
